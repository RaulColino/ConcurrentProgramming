...
### Description
The simulation's logic would be as follows: a race circuit is generated from separate parts.Each part has N units of distance. Then, each vehicle with its technical specifications is created. The race judge checks that everything on the starting grid is OK, and if it's all OK, he gives the starting signal. The vehicles are then invoked every 5 seconds, making it's moves concurrently.
